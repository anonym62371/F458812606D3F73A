...
public:
  ...
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::vector<DATATYPE>& a_searchResult, int* node_accessed = nullptr, int* leaf_accessed = nullptr) const;
...
protected:
  ...
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::vector<DATATYPE>& ret, int* node_accessed = nullptr, int* leaf_accessed = nullptr) const;
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::vector<DATATYPE>& ret, int* node_accessed, int* leaf_accessed) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  int foundCount = 0;
  Search(m_root, &rect, foundCount, ret, node_accessed, leaf_accessed);
...
// Search in an index tree or subtree for all data retangles that overlap the argument rectangle.
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::vector<DATATYPE>& ret, int* node_accessed, int* leaf_accessed) const
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      ...
      if(node_accessed) (*node_accessed)++;
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search(a_node->m_branch[index].m_child, a_rect, a_foundCount, ret, node_accessed, leaf_accessed))
        {
  ...
  else
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      ...
      if(leaf_accessed) (*leaf_accessed)++;
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
...
int RTREE_QUAL::ResidualSearch(const Node* node, const typename RTREE_QUAL_PRIME::Rect* rect_prime, const OUTRA_RECT* ovlp_rect, std::vector<int>& ret) const
{
  ...
  return 0; // TODO: just to kill the warnings
}
