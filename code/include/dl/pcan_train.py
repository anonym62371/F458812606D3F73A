...
parser.add_argument('--negatives_per_query', type=int, default=9, help='Number of definite negatives in each training tuple [default: 18]')
parser.add_argument('--max_epoch', type=int, default=5, help='Epoch to run [default: 20]')
...
parser.add_argument('--num_obj', type=int, default=100)
parser.add_argument('--rotate_pc', type=int, default=0)
parser.add_argument('--train_half', type=int, default=0)
...
# NUM_OBJ = FLAGS.num_obj
ROTATE_PC = FLAGS.rotate_pc
if ROTATE_PC > 0:
    ROTATE_PC_BOOL = True
else:
    ROTATE_PC_BOOL = False
TRAIN_HALF = FLAGS.train_half

TRAIN_FILE = DAT_PATH + '/db_pointcloud_' + RUN_ARG + '/training_queries.pickle'
TEST_FILE = DAT_PATH + '/db_pointcloud_' + RUN_ARG + '/test_queries.pickle'
DATA_FILE = DAT_PATH + '/db_pointcloud_' + RUN_ARG + '/point_cloud.bin'

POINT_CLOUDS, NUM_OBJ = load_3dor_pc(DATA_FILE, NUM_POINTS)
print('Point clouds loaded of number', NUM_OBJ)

TEST_EVERY = NUM_OBJ // 50
SAVE_EVERY = NUM_OBJ // 5
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            log_string("EPOCH completed in " + str(epoch_time) + " (s), model saved in file: " + save_path)

            total_time += epoch_time

        with open(os.path.join(LOG_DIR, "stat.txt"), "w") as handle:
            handle.write("index_time=" + str(total_time))
...
def train_one_epoch(sess, ops, train_writer, test_writer, epoch, saver):
    ...
    # Shuffle train files
    train_file_idxs = np.array(list(TRAINING_QUERIES.keys()))
    np.random.shuffle(train_file_idxs)

    print('train_file_idxs:', train_file_idxs)
    ...
    for i in range(len(train_file_idxs)//BATCH_NUM_QUERIES):
        ...
        for j in range(BATCH_NUM_QUERIES):
            ...
            if(len(TRAINING_LATENT_VECTORS)==0):
                ...
                q_tuples.append(get_query_tuple(POINT_CLOUDS, batch_keys[j], POSITIVES_PER_QUERY, NEGATIVES_PER_QUERY, TRAINING_QUERIES, hard_neg=[], other_neg=True, rotate=ROTATE_PC_BOOL))

            elif(len(HARD_NEGATIVES.keys())==0):
                query = get_feature_representation(batch_keys[j], sess, ops)
                ...
                q_tuples.append(get_query_tuple(POINT_CLOUDS, batch_keys[j], POSITIVES_PER_QUERY, NEGATIVES_PER_QUERY, TRAINING_QUERIES, hard_negs, other_neg=True, rotate=ROTATE_PC_BOOL))
        ...
        if(i % TEST_EVERY == (TEST_EVERY // 2)):
            test_file_idxs = np.array(list(TEST_QUERIES.keys()))
            ...
            for eval_batch in range(eval_batches):
                ...
                for e_tup in range(BATCH_NUM_QUERIES):
                    if(len(TEST_QUERIES[eval_keys[e_tup]]["positives"])<POSITIVES_PER_QUERY):
                        ...
                        break
                    eval_tuples.append(get_query_tuple(POINT_CLOUDS, eval_keys[e_tup], POSITIVES_PER_QUERY, NEGATIVES_PER_QUERY, TEST_QUERIES, hard_neg=[], other_neg=True))
        ...
        if(i % SAVE_EVERY == (SAVE_EVERY // 2)):
            save_path = saver.save(sess, os.path.join(LOG_DIR, "model.ckpt"))
            log_string("Model saved in file: %s" % save_path)


def get_feature_representation(dict_key, sess, ops):
    is_training=False
    queries = [POINT_CLOUDS[dict_key]]
    queries= np.expand_dims(queries,axis=1)
...
def get_latent_vectors(sess, ops, dict_to_process):
    is_training=False
    train_file_idxs = np.array(list(dict_to_process.keys()))
    ...
    for q_index in range(len(train_file_idxs)//batch_num):
        ...
        for index in file_indices:
            queries.append(POINT_CLOUDS[index])
        queries = np.array(queries)
