...
def load_3dor_pc(filename, num_pts):
    #returns Nx3 matrix
    # pc = np.fromfile(filename, dtype=np.float32)
    pc = np.fromfile(filename, dtype=np.float64)

    # if pc.shape[0] != num_pcs * num_pts * 3:
    if pc.shape[0] % (num_pts * 3) != 0:
        print("Error in pointcloud shape")
        return np.array([])

    num_pcs = pc.shape[0] // num_pts // 3
    pc = np.reshape(pc, (num_pcs, num_pts, 3))
    return pc, num_pcs
...
def get_query_tuple(POINT_CLOUDS, dict_key, num_pos, num_neg, QUERY_DICT, hard_neg=[], other_neg=False, rotate=False):
	#get query tuple for dictionary entry
	#return list [query,positives,negatives]

	# query=load_pc_file(dict_value["query"]) #Nx3
	query = POINT_CLOUDS[dict_key] #Nx3
	if rotate:
		query = rotate_point_cloud(np.expand_dims(query, axis=0))
		query = np.squeeze(query)

	dict_value = QUERY_DICT[dict_key]
	...
	positives = []
	for i in range(num_pos):
		positives.append(POINT_CLOUDS[dict_value["positives"][i]])
	...
	if (len(hard_neg) == 0):
		for i in range(num_neg):
			negatives.append(POINT_CLOUDS[dict_value["negatives"][i]])
			neg_indices.append(dict_value["negatives"][i])
	else:
		for i in hard_neg:
			negatives.append(POINT_CLOUDS[i])
			neg_indices.append(i)
		j = 0
		while (len(negatives) < num_neg):
			if not dict_value["negatives"][j] in hard_neg:
				negatives.append(POINT_CLOUDS[dict_value["negatives"][j]])
				neg_indices.append(dict_value["negatives"][j])
			j += 1
	...
	#For Quadruplet Loss
		...
		neg2 = POINT_CLOUDS[possible_negs[0]]
		if rotate:
			neg2 = rotate_point_cloud(np.expand_dims(neg2, axis=0))
			neg2 = np.squeeze(neg2)
